...
export default function AvukatContactForm() {
  const [form, setForm] = useState({ name: "", office: "", phone: "" });
  const [submitted, setSubmitted] = useState(false);

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const msg = `Merhaba, avukatlık bürosu için web sitesi tanıtımı hakkında bilgi almak istiyorum.\n\nAdım: ${form.name}\nBüro Adı: ${form.office}\nTelefon: ${form.phone}`;
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-semibold text-gray-700" htmlFor="law-name">
          Adınız Soyadınız
        </label>
        <input
          id="law-name"
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-semibold text-gray-700" htmlFor="law-office">
          Büro / Ortaklık Adı
        </label>
        <input
          id="law-office"
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-semibold text-gray-700" htmlFor="law-phone">
          Telefon Numaranız
        </label>
        <input
          id="law-phone"
      ...
      <p className="text-center text-xs text-gray-400">
        Bilgilerinizi yalnızca size ulaşmak için kullanıyoruz.
      </p>
    </form>
  );
}
